...
class Grid(object):
    ...
    def find_all_possibilities(self):
        ...
        # when one is found, find its possible entries
    ...
    def print_line(self, x):
        print "|", self.space(x, 0), self.space(x, 1), self.space(x, 2), "|", self.space(x, 3), self.space(x,
                                                                                                           4), self.space(
            x, 5), "|", self.space(x, 6), self.space(x, 7), self.space(x, 8), "|"
...
class Puzzle(object):
    ...
    def solve(self):
        ...
        while self.possible_states.empty() != True:
            ...
            if current_grid.is_completed():
                print "Puzzle solved! Yay! Solution:"
            ...
            for space in sorted(options, key=lambda k: len(options[k]), reverse=True):
                ...
                for number_available in current_grid.determine_possible_spaces(x, y):
                    ...
                    print "Solving space: (", x, ",", y, ") with", number_available
